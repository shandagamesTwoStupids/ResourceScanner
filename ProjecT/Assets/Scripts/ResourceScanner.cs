	...
	private void BuildResourceList (Scene targetScene)
	{
		...
		for (int i = 0; i < sceneObjects.Length; i++) {
			...
			for (int j = 0; j < renderers.Length; j++) {
				...
				Debug.Log (skin.sharedMesh);
				if (skin != null && skin.sharedMesh != null) {
					AddResource (skin.sharedMesh, skin.sharedMesh.name, MESH, targetScene.name);
				}
					
//				//lightmapIndex
//				LineRenderer line = renderer as LineRenderer;
//				if (line != null && line.lightmapIndex != null) {
//					
//				}

				SpriteRenderer spriteRenderer = renderer as SpriteRenderer;
				if (spriteRenderer != null && spriteRenderer.sprite != null) {
					AddResource (spriteRenderer, spriteRenderer.name, SPRITE, targetScene.name);
				}

//				TrailRenderer trail = renderer as TrailRenderer;
//				if (trail != null && trail.lightmapIndex != null) {
//					
//				}
